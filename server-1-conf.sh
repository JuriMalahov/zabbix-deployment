...
hostnamectl set-hostname mon-server; /bin/bash
...
sudo iptables-save > /root/rules

echo "Добавление команды восстановления правил iptables при перезапуске в crontab"
sudo (crontab -l 2>/dev/null; echo "@reboot /sbin/iptables-restore < /root/rules") | sudo crontab -
